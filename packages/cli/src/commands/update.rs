...
pub fn run(root: &Path, args: &UpdateArgs, global: &GlobalArgs) -> Result<i32> {
    ...
    if root.join("Cargo.toml").is_file() && root.join("Cargo.lock").is_file() {
        ...
        section("Rust / Cargo — workspace (cargo update)");
        run_step("cargo", &["update".to_string()], root, quiet)?;
    }
    ...
    if let Some(uv_root) = workspace::uv_workspace_root(root) {
        ...
        section("Python / uv — workspace (lock --upgrade + sync)");
        run_step(
            "uv",
        ...
        run_step("uv", &["sync".to_string()], &uv_root, quiet)?;
    } else {
    ...
    if !go_roots.is_empty() {
        ...
        for module in &go_roots {
            ...
            if workspace::go_uses_tool_fast_path(module) {
            ...
            } else if workspace::go_full_graph_enabled() {
                ...
                section(&format!("Go — {label} (go get -u + tidy)"));
                run_step("go", &["get".to_string(), "-u".to_string()], module, quiet)?;
            }
            run_step(
                "go",
    ...
    if workspace::uv_workspace_root(root).is_some() {
        runner::ensure_installed("uv", root)?;
        run_step("uv", &["sync".to_string()], root, quiet)?;
    } else {
        run_step(
            "moon",
    ...
    if root.join("go.work").is_file() {
        runner::ensure_installed("go", root)?;
        run_step("go", &["work".to_string(), "sync".to_string()], root, quiet)?;
    }
